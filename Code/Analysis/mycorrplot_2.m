...
%%
 function  [Rvals, Pvals]= mycorrplot_2(X,Y,xnames,ynames,type, plotson)
        ...
        %  put on lebels on the margin boxes
         if i==1 % Y- labels on the left; names of input X           
             if max(cellfun('length',xnames))<=round(50/p1) %%%%% Rotate if string is too long
                  h=text(-1.5,0,xnames{j},'fontsize',15,'fontweight','bold');
                set(h, 'rotation', 90,'HorizontalAlignment','center');
             else
                  h=text(-1.07,0,xnames{j},'fontsize',15,'fontweight','bold','HorizontalAlignment','right');
             end
         end
          if j==p1 % X- labels on the bottom; names of input Y                            
                if max(cellfun('length',ynames))>=round(50/p2) %%%%% Rotate if string is too long
                     h=text(0,-1.07,ynames{i},'fontsize',15,'fontweight','bold','HorizontalAlignment','left');
                    set(h, 'rotation', -90)
                else
                     h=text(0,-1.5,ynames{i},'fontsize',15,'fontweight','bold','HorizontalAlignment','Center');
                end
          end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    end
